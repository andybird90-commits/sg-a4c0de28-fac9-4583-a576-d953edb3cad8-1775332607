...
export const ProjectHistoryPanel: FC<ProjectHistoryPanelProps> = ({
  ...
  return (
    <Card className="bg-slate-950/60 border-slate-800" style={{ backgroundColor: "#f3f4f6" }}>
      ...
      <CardContent className="pt-0" style={{ backgroundColor: "#f3f4f6" }}>
        {events.length === 0 ?
          ...
          <li key={event.id} className="flex gap-3">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[11px] text-slate-400" style={{ color: "#1a1a1a" }}>
                      {formatDate(event.date)}
                    </span>
                    {event.meta &&
                <span className="rounded-full bg-slate-900 px-2 py-[1px] text-[10px] text-slate-300 border border-slate-700">
                        {event.meta}
                      </span>
                }
                  </div>
