      ...
      <CardHeader className="flex flex-row items-start justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex flex-col items-end gap-1 text-right">
          ...
          {isAwaitingClientReview && (
            <span className="mt-1 inline-flex items-center rounded-full border border-purple-400/60 bg-purple-50 px-2 py-[2px] text-[11px] font-medium text-purple-700">
      ...
      <CardContent className="space-y-4 pt-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          {scores.map((score) => (
            <div
              key={score.label}
              className="rounded-xl bg-slate-700 px-4 py-3 text-slate-50"
            >
        ...
        {missingItems.length > 0 && (
          <div className="mt-2 rounded-xl bg-slate-800 px-4 py-3 text-slate-50">
